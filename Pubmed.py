...
def pymedd(keyword,
    ...
    PMIDs_new = []
    for i in range(len(PMIDs_list)):
        sep = '\n'
        PMIDs_new.append(PMIDs_list[i].split(sep, 1)[0])

    return PMIDs_new, number_of_results
...
def main():
    ...
    """
    test = []
    for i in range(len(np.asarray(output))):
        test.append((np.asarray(output)[i]))
    print(test)
    """
